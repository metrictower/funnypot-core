...
serves its own 404. `symfony/yaml` is only needed by the compiler — it serializes the compiler's
inputs for both the in-repo `bin/funnypot build` (`compile-ai`) and the pinned full rebuild
(`bin/funnypot build-corpus`, see [`docs/CORPUS-PIPELINE.md`](docs/CORPUS-PIPELINE.md)); the runtime
loads none of it. It is pinned to an exact version because those bytes are part of the
compiled-artifact ABI (see the zero-drift law below). See [`SPEC.md`](SPEC.md) and
...
`funnypot build` orchestrator that encodes the whole DAG in order (`compile-ai` → `compile-wp` →
`compile-emulators` → `compile-routes` → `compile-params` → `merge-routes` → `build-manifest` → `map`; `merge-routes` is
...
### WordPress plugin/theme version decoys

Mass WordPress scanners (WPScan, nuclei-wordfence, Plecost) enumerate installs by fetching a
metadata file and reading a version out of it: `GET /wp-content/plugins/<slug>/readme.txt`
(`Stable tag:`) and `GET /wp-content/themes/<slug>/style.css` (`Version:`). The compiled corpus
already serves a generic 200 for a long tail of plugin readmes, but with no extractable version. A
curated set of high-value plugins/themes — `resources/wordpress-vuln-catalog.php`, read through
`Funnypot\Core\WordPress\VulnCatalog` — is served instead with a **real max-vulnerable version**
(the last version its CVE affects, primary-source-verified, never invented) plus the slug, so the
scanner extracts a version its `compare_versions()` bound accepts and records a confirmed
vulnerable install. `bin/funnypot compile-wp` regenerates one `new_page` route per curated slug into
`templates/generated-wp/` (its own dir, so `compile-ai` clearing `templates/generated` never erases
it); a slug that already has a corpus readme key gets the versioned bundle appended with a heavy
`weight` so it wins the persona pick. Re-run after a catalog change, then `composer build`.
`VulnCatalog::has()` is the curated-set (CVE-tag) hint — not a serve gate: the corpus long tail
still serves its generic 200.
...
| **Kubernetes / etcd** | Unauthenticated etcd keyspace dump: v2 `GET /v2/keys*` (recursive registry tree + `/registry/secrets/…` leaves, single-node subpath reads) · v3 `GET`/`POST /v3/kv/range` (base64 kvs) | A believable k8s keyspace with inert synthetic ServiceAccount honeytokens — v2 upgrades the terse corpus stub via an `owns_path` override while reproducing the scanner witness (`"node":`/`"key":`/json/200); v3 is a new surface, request-body-independent, keys/values base64 per the v3 byte-array API |
...
| **Static file & config leaks** *(route tier)* | config & secret files, cloud/AWS creds, `.env`, logs, API docs / Swagger, VCS metadata (`.git` HEAD/logs/packed-refs/refs, `.svn`, `.hg`, `.bzr`, `CVS/Entries` — coherent shas/refs/branch), CMS configs (TYPO3 `typo3conf/` + `localconf.php` · Joomla webservice config API `/api/index.php/v1/config/application` + `/api/v1` alias, CVE-2023-23752 — leaks the seeded DB credentials as JSON:API `data[].attributes`), backups, `phpinfo`, directory listings | The long tail scanners crawl for — served as plausible disclosure pages |

> A generated visual of this inventory lives at [`docs/DECOY-MAP.md`](docs/DECOY-MAP.md) — a Mermaid
> overview plus a per-family table, regenerated by `bin/funnypot map` from the compiled manifest.
> Run `bin/funnypot map --family=NAME` for one family's routes and outbound links. The table below is
> generated from the manifest (measurable facts only — the taxonomy above stays hand-owned).

<!-- GENERATED-DECOY-INVENTORY:START -->
<!-- Generated by `bin/funnypot map` from resources/compiled/funnypot-manifest.php. Do not edit by hand. -->

**Decoy inventory** (manifest schema 1, measurable facts only):

| Metric | Count |
|---|---|
| Authored decoy records | 340 |
| Authored families | 81 |
| &nbsp;&nbsp;tier `attack` | 70 |
| &nbsp;&nbsp;tier `attack-ai` | 9 |
| &nbsp;&nbsp;tier `attack-crs` | 4 |
| &nbsp;&nbsp;tier `new-page` | 255 |
...
| Content enrichers (not route claimers) | 221 |
...
### Invisible login honeypot field

The served WordPress login surface (the GET login page, route `route-wp-login`, and the POST
credential-oracle error page, attack `attack-wp-login`) carries an extra hidden contact `<input>`. It
is removed from the render, the tab order, the accessibility tree and browser autofill (a `display:none`
wrapper plus `tabindex="-1"`, `aria-hidden="true"`, `autocomplete="off"`), so no real user ever fills
it — but a dumb form-filling bot does. A non-empty value on submit is therefore a zero-false-positive
scripted-bot signal.

The field name is a per-request `{{pick:login_decoy_field:…}}` from a closed list of ordinary
contact-field names, keyed on the render seed `crc32(Config::seedFor($r))` (per-client-IP where the app
injects that closure, per-host by default). It is not a fleet-wide constant and carries no honeypot
tell. Because `seedFor` has no per-request nonce, one source's GET-render field name equals its
POST-submit name, so render and detect agree.

Core only **renders** the field; detection is a pure, serve-neutral helper,
`Support\LoginDecoyField` — `expectedName()` re-derives the same name with the same seed, and
`isTripped()` reports whether the field arrived non-empty. It produces no `Verdict`/`Detection` and
never enters the serve decision, so the login page is byte-identical whether or not the field is filled
and it never blocks (inert, only-upgrade-404, submitted value never reflected). The store-owning tier
(the funnypot-app controller for the box; the funnypot-wordpress plugin for a real WP install) calls
`isTripped()` outside the serve path and records the hit in its own store — core owns the detection
logic, the caller owns the store. `LoginDecoyField::DIRECTIVE` is the single source of truth both login
templates carry verbatim, so render and detect cannot drift.

How a route-tier decoy is authored — the `match` / `new_page` / `response` DSL, the closed `response`
key set (`headers | body | body_b64 | binary | binary_generator`, exactly one body source), and the
`binary_generator` contract (closed built-in registry, 64 KB inclusive ceiling, decline-to-404, the
old-runtime sentinel) — is in [`docs/ROUTE-TEMPLATES.md`](docs/ROUTE-TEMPLATES.md).
...
`git status --porcelain` over `resources/compiled` + `templates/generated` + `templates/generated-wp` + `templates/attack-ai`
...
**The YAML serializer version is part of that ABI.** `compile-ai` serializes the committed compiler
inputs with `symfony/yaml`, so the serializer *version* — not merely its PHP API — helps define the
generated bytes. It is pinned to an exact version in `require-dev` (never a caret, tilde, wildcard or
range), and a guard test (`tests/YamlSerializerPinTest.php`) fails the build if the constraint ever
floats. Upgrading it, including a security update, stays allowed but is a deliberate artifact
migration in one reviewable commit: bump the exact version and its guard, install from a clean solve,
run `composer build`, and review every changed `templates/generated`/`templates/generated-wp`/`templates/attack-ai` byte and
the downstream index/manifest/provenance diff before release. It is never an incidental `composer
update` — that unreviewed drift is exactly what the pin prevents.
