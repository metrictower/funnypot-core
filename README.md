...
| **Static file & config leaks** *(route tier)* | config & secret files, cloud/AWS creds, `.env`, logs, API docs / Swagger, VCS metadata (`.git` HEAD/logs/packed-refs/refs, `.svn`, `.hg`, `.bzr` — coherent shas/refs/branch), backups, `phpinfo`, directory listings | The long tail scanners crawl for — served as plausible disclosure pages |
