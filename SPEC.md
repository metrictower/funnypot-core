...
| `isolatedOrigin` | `false` | fail-safe origin posture and operator **intent**. `false` = embedded/inline — decoys tagged `reflects_input` (echo attacker bytes into an HTML body or a redirect `Location`) are **withheld from serving** (detection unaffected). `true` asserts a standalone origin but since v0.7.0 is **necessary, not sufficient**: a reflector also needs a live request and a `reflectorAuthorizer` returning `true` (see below) |
| `reflectClasses` | `[]` | per-reflect-class serve override, keyed by a rule's `reflect_class` (`xss` / `open-redirect` / `fs-read`) or the code-defined `param-reaction` class; missing key ⇒ enabled. AND-composed into the serve decision so it can only **subtract** — an isolated origin can turn one class off, but no value re-enables reflection on an embedded host |
| `reflectorAuthorizer` | `null` | `fn(RequestContext, string $reflectClass):bool` — the request-bound **evidence** term. A reflector serves only when `isolatedOrigin && (reflectClasses[class] ?? true) && request-present && reflectorAuthorizer(request, class) === true`. Fail-closed like `gate`: `null` ⇒ no reflector serves (an isolated origin that wires none is **safe-off**), only literal `true` authorizes, a `Throwable` ⇒ suppressed. Must return `true` only from a **server-derived fact a client cannot supply** (e.g. a static FastCGI param from an exact deception vhost) — never from `Host`/`X-Forwarded-*`/scheme/cookie/query/header. The position-blind `synthesize()` port carries no request, so it suppresses every reflector |
| `paramReactivity` | `false` | opt-in **bounded query-parameter reactions** on resolved decoy routes (FP-0157). `false` ⇒ no route handle carries a query intent and no response is decorated (byte-identical to before). `true` ⇒ a resolved decoy route MAY append an inert, persona-coherent reaction (file-read / redirect-notice / debug-view / command-result / search-result) when a query matches the closed intent matrix. It is a **reflecting decoy**: also gated by `serveReflector('param-reaction', request)` at classify AND synthesize, so an embedded origin (`isolatedOrigin=false`) and the position-blind port never react. Never creates a route, changes classification, weakens a matcher, or reaches a header/redirect/I/O context; the reflected value is display-only, encoded per output context. Every decline preserves the exact base response |
