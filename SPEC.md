...
| `reflectClasses` | `[]` | per-reflect-class serve override, keyed by a rule's `reflect_class` (`xss` / `open-redirect` / `fs-read`); missing key ⇒ enabled. AND-composed into the serve decision so it can only **subtract** — an isolated origin can turn one class off, but no value re-enables reflection on an embedded host |
